...
class RBTableView: NSTableView {
    ...
    @objc func copy(_ sender: AnyObject?) {
        ...
        for obj in self.associatedArrayController().selectedObjects as? [RBSpotifyItem] ?? [] {
    ...
    override func keyDown(with event: NSEvent) {
                  ...
                  flags.isEmpty
        {
            self.selectRow(row: 0)
                  ...
                  flags == .shift
        {
            self.selectRow(row: self.numberOfRows - 1)
        ...
        { // "l" key, right
            self.focusSearchResults()
                  ...
                  event.keyCode == kVK_RightArrow
        { // cmd-right, search for album
            self.browseDetailsOnRow()
        } else if flags == .command,
                  event.characters == ";"
        { // cmd-;, search for album, because cmd-L is taken?
            self.browseDetailsOnRow()
                  ...
                  flags.isEmpty
        {
            AppDelegate.appDelegate().playOrPause(self)
                  ...
                  flags.isEmpty
        {
            self.selectRow(row: selectedRow - self.nbVisibleRows() + 1)
                                          ...
                                          step: Int = 0)
    {
        ...
        let chunk: [SpotifyURIConvertible] = Array(items.prefix(self.ADD_TO_PLAYLIST_CHUNKSIZE))
        let rest: [SpotifyURIConvertible] = Array(items.dropFirst(self.ADD_TO_PLAYLIST_CHUNKSIZE))
